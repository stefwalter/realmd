...
typedef struct _SystemClosure SystemClosure;

typedef void (* PerformFunc) (RealmDbusSystemdManager *systemd,
                              GSimpleAsyncResult *res,
                              SystemClosure *service);

struct _SystemClosure {
                   ...
                   gpointer user_data)
{
	GSimpleAsyncResult *res = G_SIMPLE_ASYNC_RESULT (user_data);
	RealmDbusSystemdManager *systemd = REALM_DBUS_SYSTEMD_MANAGER (object);
	SystemClosure *service = g_simple_async_result_get_op_res_gpointer (res);
	GError *error = NULL;
                      ...
                      gpointer user_data)
{
	...
	service = g_slice_new (SystemClosure);
...
void
realm_system_enable_and_restart_service (const gchar *service_name,
                                         GDBusMethodInvocation *invocation,
                                         GAsyncReadyCallback callback,
                                         gpointer user_data)
{
...
gboolean
realm_system_enable_service_finish (GAsyncResult *result,
                                    GError **error)
{
...
void
realm_system_disable_and_stop_service (const gchar *service_name,
                                       GDBusMethodInvocation *invocation,
                                       GAsyncReadyCallback callback,
                                       gpointer user_data)
{
...
gboolean
realm_system_disable_service_finish (GAsyncResult *result,
                                     GError **error)
{
	g_return_val_if_fail (g_simple_async_result_is_valid (result, NULL,
	                      begin_service_action), FALSE);
