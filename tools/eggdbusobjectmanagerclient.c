 ...
 * @include: gio/gio.h
 *
 * #EggDBusObjectManagerClient is used to create, monitor and delete object
 ...
 *
 * If the name for a #EggDBusObjectManagerClient is not owned by anyone at
 * object construction time, the default behavior is to request the
 * message bus to launch an owner for the name. This behavior can be
 * disabled using the %EGG_DBUS_OBJECT_MANAGER_CLIENT_FLAGS_DO_NOT_AUTO_START
 * flag. It's also worth noting that this only works if the name of
 * interest is activatable in the first place. E.g. in some cases it
 * is not possible to launch an owner for the requested name. In this
 * case, #EggDBusObjectManagerClient object construction still succeeds but
 * there will be no object proxies
 * (e.g. g_dbus_object_manager_get_objects() returns the empty list) and
 * the #EggDBusObjectManagerClient:name-owner property is %NULL.
 *
 * The owner of the requested name can come and go (for example
 * consider a system service being restarted) – #EggDBusObjectManagerClient
 * handles this case too; simply connect to the #GObject::notify
 * signal to watch for changes on the #EggDBusObjectManagerClient:name-owner
 * property. When the name owner vanishes, the behavior is that
 * #EggDBusObjectManagerClient:name-owner is set to %NULL (this includes
 * emission of the #GObject::notify signal) and then
 * #GDBusObjectManager::object-removed signals are synthesized
 * for all currently existing object proxies. Since
 * #EggDBusObjectManagerClient:name-owner is %NULL when this happens, you can
 * use this information to disambiguate a synthesized signal from a
 * genuine signal caused by object removal on the remote
 * #GDBusObjectManager. Similarly, when a new name owner appears,
 * #GDBusObjectManager::object-added signals are synthesized
 * while #EggDBusObjectManagerClient:name-owner is still %NULL. Only when all
 * object proxies have been added, the #EggDBusObjectManagerClient:name-owner
 * is set to the new name owner (this includes emission of the
 * #GObject::notify signal).  Furthermore, you are guaranteed that
 * #EggDBusObjectManagerClient:name-owner will alternate between a name owner
 * (e.g. <literal>:1.42</literal>) and %NULL even in the case where
 * the name of interest is atomically replaced
 *
 * Ultimately, #EggDBusObjectManagerClient is used to obtain #GDBusProxy
 ...
 * are also emitted on the #EggDBusObjectManagerClient instance managing these
 * objects. The signals emitted are
 * #GDBusObjectManager::interface-added,
 * #GDBusObjectManager::interface-removed,
 * #EggDBusObjectManagerClient::interface-proxy-properties-changed and
 * #EggDBusObjectManagerClient::interface-proxy-signal.
 *
 * Note that all callbacks and signals are emitted in the
 * <link linkend="g-main-context-push-thread-default">thread-default main loop</link>
 * that the #EggDBusObjectManagerClient object was constructed
 * in. Additionally, the #GDBusObjectProxy and #GDBusProxy objects
 * originating from the #EggDBusObjectManagerClient object will be created in
 * the same context and, consequently, will deliver signals in the
 * same main loop.
 */

struct _EggDBusObjectManagerClientPrivate
{
  GMutex lock;
...
G_DEFINE_TYPE_WITH_CODE (EggDBusObjectManagerClient, egg_dbus_object_manager_client, G_TYPE_OBJECT,
                         G_IMPLEMENT_INTERFACE (G_TYPE_INITABLE, initable_iface_init)
                         G_IMPLEMENT_INTERFACE (G_TYPE_ASYNC_INITABLE, async_initable_iface_init)
                         G_IMPLEMENT_INTERFACE (G_TYPE_DBUS_OBJECT_MANAGER, dbus_object_manager_interface_init));

static void maybe_unsubscribe_signals (EggDBusObjectManagerClient *manager);
...
static void process_get_all_result (EggDBusObjectManagerClient *manager,
                                    GVariant          *value,
                                    const gchar       *name_owner);

static void
egg_dbus_object_manager_client_finalize (GObject *object)
{
  EggDBusObjectManagerClient *manager = EGG_DBUS_OBJECT_MANAGER_CLIENT (object);
  ...
  if (G_OBJECT_CLASS (egg_dbus_object_manager_client_parent_class)->finalize != NULL)
    G_OBJECT_CLASS (egg_dbus_object_manager_client_parent_class)->finalize (object);
}

static void
egg_dbus_object_manager_client_get_property (GObject    *object,
                                           guint       prop_id,
                                           GValue     *value,
                                           GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_CONNECTION:
      g_value_set_object (value, egg_dbus_object_manager_client_get_connection (manager));
      break;
    ...
    case PROP_NAME:
      g_value_set_string (value, egg_dbus_object_manager_client_get_name (manager));
      break;

    case PROP_FLAGS:
      g_value_set_flags (value, egg_dbus_object_manager_client_get_flags (manager));
      break;

    case PROP_NAME_OWNER:
      g_value_take_string (value, egg_dbus_object_manager_client_get_name_owner (manager));
      break;
...
static void
egg_dbus_object_manager_client_set_property (GObject       *object,
                                           guint          prop_id,
                                           const GValue  *value,
                                           GParamSpec    *pspec)
{
...
static void
egg_dbus_object_manager_client_class_init (EggDBusObjectManagerClientClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  gobject_class->finalize     = egg_dbus_object_manager_client_finalize;
  gobject_class->set_property = egg_dbus_object_manager_client_set_property;
  gobject_class->get_property = egg_dbus_object_manager_client_get_property;

  /**
   * EggDBusObjectManagerClient:connection:
   *
  ...
  /**
   * EggDBusObjectManagerClient:bus-type:
   *
   * If this property is not %G_BUS_TYPE_NONE, then
   * #EggDBusObjectManagerClient:connection must be %NULL and will be set to the
  ...
  /**
   * EggDBusObjectManagerClient:flags:
   *
  ...
  /**
   * EggDBusObjectManagerClient:object-path:
   *
  ...
  /**
   * EggDBusObjectManagerClient:name:
   *
  ...
  /**
   * EggDBusObjectManagerClient:name-owner:
   *
   * The unique name that owns #EggDBusObjectManagerClient:name or %NULL if
  ...
  /**
   * EggDBusObjectManagerClient:get-proxy-type-func:
   *
  ...
  /**
   * EggDBusObjectManagerClient:get-proxy-type-user-data:
   *
   * The #gpointer user_data to pass to #EggDBusObjectManagerClient:get-proxy-type-func.
   *
   * Since: 2.30
   */
  g_object_class_install_property (gobject_class,
  ...
  /**
   * EggDBusObjectManagerClient:get-proxy-type-destroy-notify:
   *
   * A #GDestroyNotify for the #gpointer user_data in #EggDBusObjectManagerClient:get-proxy-type-user-data.
   *
   * Since: 2.30
   */
  g_object_class_install_property (gobject_class,
  ...
  /**
   * EggDBusObjectManagerClient::interface-proxy-signal:
   * @manager: The #EggDBusObjectManagerClient emitting the signal.
  ...
  signals[INTERFACE_PROXY_SIGNAL_SIGNAL] =
    g_signal_new ("interface-proxy-signal",
                  EGG_TYPE_DBUS_OBJECT_MANAGER_CLIENT,
                  G_SIGNAL_RUN_LAST,
  ...
  /**
   * EggDBusObjectManagerClient::interface-proxy-properties-changed:
   * @manager: The #EggDBusObjectManagerClient emitting the signal.
  ...
  signals[INTERFACE_PROXY_PROPERTIES_CHANGED_SIGNAL] =
    g_signal_new ("interface-proxy-properties-changed",
                  EGG_TYPE_DBUS_OBJECT_MANAGER_CLIENT,
                  G_SIGNAL_RUN_LAST,
                  G_STRUCT_OFFSET (EggDBusObjectManagerClientClass, interface_proxy_properties_changed),
                  NULL,
                  NULL,
                  NULL,
                  G_TYPE_NONE,
  ...
  g_type_class_add_private (klass, sizeof (EggDBusObjectManagerClientPrivate));
}

static void
egg_dbus_object_manager_client_init (EggDBusObjectManagerClient *manager)
{
  manager->priv = G_TYPE_INSTANCE_GET_PRIVATE (manager,
                                               EGG_TYPE_DBUS_OBJECT_MANAGER_CLIENT,
                                               EggDBusObjectManagerClientPrivate);
...
/**
 * egg_dbus_object_manager_client_new_sync:
 ...
 *
 * Creates a new #EggDBusObjectManagerClient object.
 *
 * This is a synchronous failable constructor - the calling thread is
 * blocked until a reply is received. See egg_dbus_object_manager_client_new()
 * for the asynchronous version.
 *
 * Returns: (transfer full) (type EggDBusObjectManagerClient): A
 *   #EggDBusObjectManagerClient object or %NULL if @error is set. Free
 *   with g_object_unref().
 *
 * Since: 2.30
 */
GDBusObjectManager *
egg_dbus_object_manager_client_new_sync (GDBusConnection               *connection,
                                       GDBusObjectManagerClientFlags  flags,
                                       const gchar                   *name,
                                       ...
                                       GError                       **error)
{
  ...
  initable = g_initable_new (EGG_TYPE_DBUS_OBJECT_MANAGER_CLIENT,
                             cancellable,
                             error,
...
/**
 * egg_dbus_object_manager_client_new:
 * @connection: A #GDBusConnection.
 * @flags: Zero or more flags from the #GDBusObjectManagerClientFlags enumeration.
 ...
 *
 * Asynchronously creates a new #EggDBusObjectManagerClient object.
 *
 * This is an asynchronous failable constructor. When the result is
 * ready, @callback will be invoked in the
 * <link linkend="g-main-context-push-thread-default">thread-default main loop</link>
 * of the thread you are calling this method from. You can
 * then call egg_dbus_object_manager_client_new_finish() to get the result. See
 * egg_dbus_object_manager_client_new_sync() for the synchronous version.
 *
 * Since: 2.30
 */
void
egg_dbus_object_manager_client_new (GDBusConnection               *connection,
                                  GDBusObjectManagerClientFlags  flags,
                                  const gchar                   *name,
                                  const gchar                   *object_path,
                                  EggDBusProxyTypeFunc             get_proxy_type_func,
                                  ...
                                  gpointer                       user_data)
{
  ...
  g_async_initable_new_async (EGG_TYPE_DBUS_OBJECT_MANAGER_CLIENT,
                              G_PRIORITY_DEFAULT,
                              cancellable,
                              callback,
                              user_data,
...
/**
 * egg_dbus_object_manager_client_new_finish:
 * @res: A #GAsyncResult obtained from the #GAsyncReadyCallback passed to egg_dbus_object_manager_client_new().
 * @error: Return location for error or %NULL.
 *
 * Finishes an operation started with egg_dbus_object_manager_client_new().
 *
 * Returns: (transfer full) (type EggDBusObjectManagerClient): A
 *   #EggDBusObjectManagerClient object or %NULL if @error is set. Free
 *   with g_object_unref().
 ...
 */
GDBusObjectManager *
egg_dbus_object_manager_client_new_finish (GAsyncResult   *res,
                                         GError        **error)
{
  GObject *object;
...
/**
 * egg_dbus_object_manager_client_new_for_bus_sync:
 * @bus_type: A #GBusType.
 * @flags: Zero or more flags from the #GDBusObjectManagerClientFlags enumeration.
 * @name: The owner of the control object (unique or well-known name).
 * @object_path: The object path of the control object.
 ...
 * @error: Return location for error or %NULL.
 *
 * Like egg_dbus_object_manager_client_new_sync() but takes a #GBusType instead
 * of a #GDBusConnection.
 *
 * This is a synchronous failable constructor - the calling thread is
 * blocked until a reply is received. See egg_dbus_object_manager_client_new_for_bus()
 * for the asynchronous version.
 *
 * Returns: (transfer full) (type EggDBusObjectManagerClient): A
 *   #EggDBusObjectManagerClient object or %NULL if @error is set. Free
 ...
 */
GDBusObjectManager *
egg_dbus_object_manager_client_new_for_bus_sync (GBusType                       bus_type,
                                               GDBusObjectManagerClientFlags  flags,
                                               const gchar                   *name,
                                               const gchar                   *object_path,
                                               EggDBusProxyTypeFunc             get_proxy_type_func,
...
/**
 * egg_dbus_object_manager_client_new_for_bus:
 * @bus_type: A #GBusType.
 * @flags: Zero or more flags from the #GDBusObjectManagerClientFlags enumeration.
 * @name: The owner of the control object (unique or well-known name).
 * @object_path: The object path of the control object.
 ...
 * @user_data: The data to pass to @callback.
 *
 * Like egg_dbus_object_manager_client_new() but takes a #GBusType instead of a
 * #GDBusConnection.
 *
 * This is an asynchronous failable constructor. When the result is
 * ready, @callback will be invoked in the
 * <link linkend="g-main-context-push-thread-default">thread-default main loop</link>
 * of the thread you are calling this method from. You can
 * then call egg_dbus_object_manager_client_new_for_bus_finish() to get the result. See
 * egg_dbus_object_manager_client_new_for_bus_sync() for the synchronous version.
 *
 * Since: 2.30
 */
void
egg_dbus_object_manager_client_new_for_bus (GBusType                       bus_type,
                                          GDBusObjectManagerClientFlags  flags,
                                          const gchar                   *name,
                                          const gchar                   *object_path,
                                          EggDBusProxyTypeFunc             get_proxy_type_func,
 ...
 * @res: A #GAsyncResult obtained from the #GAsyncReadyCallback passed to egg_dbus_object_manager_client_new_for_bus().
 * @error: Return location for error or %NULL.
 *
 * Finishes an operation started with egg_dbus_object_manager_client_new_for_bus().
 *
 * Returns: (transfer full) (type EggDBusObjectManagerClient): A
 *   #EggDBusObjectManagerClient object or %NULL if @error is set. Free
 *   with g_object_unref().
 ...
 */
GDBusObjectManager *
egg_dbus_object_manager_client_new_for_bus_finish (GAsyncResult   *res,
                                                 GError        **error)
{
  GObject *object;
  GObject *source_object;
...
/**
 * egg_dbus_object_manager_client_get_connection:
 ...
 */
GDBusConnection *
egg_dbus_object_manager_client_get_connection (EggDBusObjectManagerClient *manager)
{
  GDBusConnection *ret;
  g_return_val_if_fail (EGG_IS_DBUS_OBJECT_MANAGER_CLIENT (manager), NULL);
...
/**
 * egg_dbus_object_manager_client_get_name:
 * @manager: A #EggDBusObjectManagerClient
 *
 ...
 */
const gchar *
egg_dbus_object_manager_client_get_name (EggDBusObjectManagerClient *manager)
{
  const gchar *ret;
  g_return_val_if_fail (EGG_IS_DBUS_OBJECT_MANAGER_CLIENT (manager), NULL);
  g_mutex_lock (&manager->priv->lock);
...
/**
 * egg_dbus_object_manager_client_get_flags:
 * @manager: A #EggDBusObjectManagerClient
 *
...
egg_dbus_object_manager_client_get_flags (EggDBusObjectManagerClient *manager)
{
  GDBusObjectManagerClientFlags ret;
  g_return_val_if_fail (EGG_IS_DBUS_OBJECT_MANAGER_CLIENT (manager), G_DBUS_OBJECT_MANAGER_CLIENT_FLAGS_NONE);
  g_mutex_lock (&manager->priv->lock);
...
/**
 * egg_dbus_object_manager_client_get_name_owner:
 * @manager: A #EggDBusObjectManagerClient.
 *
 * The unique name that owns the name that @manager is for or %NULL if
 * no-one currently owns that name. You can connect to the
 * #GObject::notify signal to track changes to the
 * #EggDBusObjectManagerClient:name-owner property.
 *
 ...
 */
gchar *
egg_dbus_object_manager_client_get_name_owner (EggDBusObjectManagerClient *manager)
{
  gchar *ret;
  g_return_val_if_fail (EGG_IS_DBUS_OBJECT_MANAGER_CLIENT (manager), NULL);
  g_mutex_lock (&manager->priv->lock);
...
signal_cb (GDBusConnection *connection,
           ...
           gpointer         user_data)
{
  EggDBusObjectManagerClient *manager = EGG_DBUS_OBJECT_MANAGER_CLIENT (user_data);
...
static void
subscribe_signals (EggDBusObjectManagerClient *manager,
                   const gchar *name_owner)
{
  GError *error = NULL;
  GVariant *ret;

  g_return_if_fail (EGG_IS_DBUS_OBJECT_MANAGER_CLIENT (manager));
...
static void
maybe_unsubscribe_signals (EggDBusObjectManagerClient *manager)
{
  g_return_if_fail (EGG_IS_DBUS_OBJECT_MANAGER_CLIENT (manager));
  ...
  if (g_strcmp0 (old_name_owner, new_name_owner) != 0)
    {
      ...
      for (l = proxies; l != NULL; l = l->next)
        {
          EggDBusObjectProxy *object_proxy = EGG_DBUS_OBJECT_PROXY (l->data);
...
initable_init (GInitable     *initable,
               ...
               GError       **error)
{
  EggDBusObjectManagerClient *manager = EGG_DBUS_OBJECT_MANAGER_CLIENT (initable);
  gboolean ret;
  GVariant *value;
...
static void
add_interfaces (EggDBusObjectManagerClient *manager,
                const gchar       *object_path,
                GVariant          *ifaces_and_properties,
                const gchar       *name_owner)
{
  EggDBusObjectProxy *op;
  gboolean added;
  GVariantIter iter;
  const gchar *interface_name;
  GVariant *properties;
      ...
      if (manager->priv->get_proxy_type_func != NULL)
        {
          ...
          g_warn_if_fail (g_type_is_a (object_proxy_type, EGG_TYPE_DBUS_OBJECT_PROXY));
        }
      else
        {
          object_proxy_type = EGG_TYPE_DBUS_OBJECT_PROXY;
        }
                              ...
                              &properties))
    {
      ...
      if (interface_proxy == NULL)
      ...
      else
        {
          ...
          egg_dbus_object_proxy_add_interface (op, interface_proxy);
          if (!added)
...
static void
remove_interfaces (EggDBusObjectManagerClient   *manager,
                   const gchar         *object_path,
      ...
      for (n = 0; interface_names != NULL && interface_names[n] != NULL; n++)
        {
          ...
          egg_dbus_object_proxy_remove_interface (op, interface_names[n]);
          if (interface != NULL)
            {
...
static void
process_get_all_result (EggDBusObjectManagerClient *manager,
                        GVariant          *value,
                        const gchar       *name_owner)
{
  GVariant *arg0;
...
static const gchar *
egg_dbus_object_manager_client_get_object_path (GDBusObjectManager *_manager)
{
  EggDBusObjectManagerClient *manager = EGG_DBUS_OBJECT_MANAGER_CLIENT (_manager);
  return manager->priv->object_path;
}

static GDBusObject *
egg_dbus_object_manager_client_get_object (GDBusObjectManager *_manager,
                                         const gchar        *object_path)
{
  EggDBusObjectManagerClient *manager = EGG_DBUS_OBJECT_MANAGER_CLIENT (_manager);
  GDBusObject *ret;
...
egg_dbus_object_manager_client_get_interface  (GDBusObjectManager  *_manager,
                                             const gchar         *object_path,
                                             const gchar         *interface_name)
{
  GDBusInterface *ret;
...
static GList *
egg_dbus_object_manager_client_get_objects (GDBusObjectManager *_manager)
{
  EggDBusObjectManagerClient *manager = EGG_DBUS_OBJECT_MANAGER_CLIENT (_manager);
  GList *ret;
...
dbus_object_manager_interface_init (GDBusObjectManagerIface *iface)
{
  ...
  iface->get_objects     = egg_dbus_object_manager_client_get_objects;
  iface->get_object      = egg_dbus_object_manager_client_get_object;
  iface->get_interface   = egg_dbus_object_manager_client_get_interface;
}
